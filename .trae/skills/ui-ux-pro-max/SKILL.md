...
> **Note:** On Windows, use `python` instead of `python3` to run scripts (e.g., `python .trae/skills/ui-ux-pro-max/scripts/search.py` instead of `python3 .trae/skills/ui-ux-pro-max/scripts/search.py`).
...
```bash
python3 .trae/skills/ui-ux-pro-max/scripts/search.py "list performance navigation" --stack react-native
```
...
- 默认图标库使用 **Phosphor (`@phosphor-icons/react`)**。`.trae/skills/ui-ux-pro-max/data/icons.csv` 中列出的只是常用推荐图标，不是完整集合。
- 当推荐表中找不到合适的图标时：
